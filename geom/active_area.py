...
Notes:
------
The area calculations are based on geometric projections, ie we assume
that the beam is perfectly parrallel (no divergence)

Todo:
-----
* Have option of defining the detector polygon from an 
  arbirary set of pixels (e.g. roi) on an image detector... 
"""
##########################################################################
import numpy as num
import types
...
from pyxrs.common import cosd, sind, tand
from pyxrs.common import arccosd, arcsind, arctand
from pyxrs.common import cartesian_mag, cartesian_angle

from pyxrs.geom.polygon import poly_area, poly_area_num, inner_polygon
from pyxrs.geom.polygon import plot_polygon, plot_points, plot_circle
...
def active_area(nm,ki=num.array([0.,1.,0.]),kr=num.array([0.,1.,0.]),
    ...
    * A_int  = area of sample illuminated within the detector projection 
    ...
    Use to correct scattering data for area effects, including spilloff, i.e.
      I_corrected = I/A_ratio   # corrected scattering intensity
      A_ratio = A_int/A_beam    # intersection area / beam area
      I = Idet/Io               # raw scattering intensity normalized to Io
